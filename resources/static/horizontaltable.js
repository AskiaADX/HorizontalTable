   ...
   */
    function stepByStepColumns (that) {
    	...
    	for (var i = (that.nbLoopItems); i-- > 0; ) {
    		...
    		for (var j = divs.length; j-- > 1; ) {
                if (hasClass(divs[j],'date') && (checkAnswersDate(divs[j]) || (that.arrInputCodesHiddenQuestions.indexOf(parseInt(divs[j].getAttribute('data-class').split('_')[1], 10)) >= 0)) ) {
					nbDataFound = nbDataFound + 1;
                } else if (hasClass(divs[j],'open') && (checkAnswersOpen(divs[j]) || (that.arrInputCodesHiddenQuestions.indexOf(parseInt(divs[j].getAttribute('data-class').split('_')[1], 10)) >= 0)) ) {
                    nbDataFound = nbDataFound + 1;
                } else if (hasClass(divs[j],'numeric') && (checkAnswersNumeric(divs[j]) || (that.arrInputCodesHiddenQuestions.indexOf(parseInt(divs[j].getAttribute('data-class').split('_')[1], 10)) >= 0)) ) {
                    nbDataFound = nbDataFound + 1;
            ...
            if ((nbDataFound !== (divs.length - 1)) && (lastDataFound  === 0)) {
                ...
                nextTr.forEach(function(item){
                    addClass(item,'display');
                });
            } else {
                if (nextTr[1].style.visibility === 'hidden') {
                    if (that.stepByStep) {
                        nextTr.forEach(function(item){
    ...
    function autoSubmitForm (that) {
    	...
    	for (var i = (that.nbLoopItems); i-- > 0; ) {
            ...
            // Iterate td backwards
    		for (var j = divs.length; j-- > 1; ) {
                if (hasClass(divs[j],'open') || hasClass(divs[j],'numeric') || hasClass(divs[j],'multiple')) {
                    result = false;
                	break;
                }
                if (hasClass(divs[j],'date') && (checkAnswersDate(divs[j]) || (that.arrInputCodesHiddenQuestions.indexOf(parseInt(divs[j].getAttribute('data-class').split('_')[1], 10)) >= 0)) ) {
            ...
            if ((nbDataFound !== (divs.length - 1)) || (result === false)) {
  				result = false;
                break;
            }
        }
   ...
   */
    function HorizontalTable (options) {	
        ...
        window.addEventListener("resize", function() {
            ...
            debounceMakeResponsesSameHeight(options);
        });
